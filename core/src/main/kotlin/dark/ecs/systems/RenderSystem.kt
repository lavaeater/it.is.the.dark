        ...
        batch.use {
            ...
            for (blobList in BlobGrouper.blobGroups) {
                for ((index, blobEntity) in blobList.withIndex()) {
                    ...
                    shapeDrawer.filledCircle(blobPosition, 1f, Color(1f-health.normalizedValue,health.normalizedValue, 1f-health.normalizedValue, health.normalizedValue))
            ...
            for ((position, normalizedEnergy) in foodRenderStuff) {
                shapeDrawer.filledCircle(position, 2.5f * normalizedEnergy, Color(1f-normalizedEnergy, normalizedEnergy, 1f-normalizedEnergy, normalizedEnergy))
            }
        }
        fbo.end()
        batch.use {
            ...
            batch.draw(textureRegion, camera.position.x - camera.viewportWidth / 2f, camera.position.y - camera.viewportHeight / 2f, camera.viewportWidth, camera.viewportHeight)
            batch.shader = null
        }
    }
}